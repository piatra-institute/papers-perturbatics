...
- Numbers current as of the v7 (2026-07-21) run, after the generator guard: instances are kept only when the frozen rest path meets the wall, so every blocked episode genuinely forces a detour. This shifted about two dozen map and boundary numbers from the v6 values (immaterial to the headline, pooled separation 0.945 either way); the persona's pure-channel shares are invariant.
- Separation, battery-shaped: every pairwise AUROC 0.50 at rest. Per probe: move exposes the script (planner and reactive vs script both 1.0) and leaves planner vs reactive at 0.50 (trajectory-identical); block exposes the planner (0.79 vs both) and leaves reactive vs script at 0.50. Planner vs script pooled 0.945. Mean probe evidence: planner +30.0, script -31.9, reactive +26.4.
- Realization map (dual, do-Shapley): evidence phi_E goal register 0.70, planner 0.37, persona 0.23 (= DECL_FRAC/(1+DECL_FRAC), the cheap-talk dial, by construction); capacity phi_C goal register 0.81, planner 0.25, persona 0.00 (no motor channel, by design). Legibility L: persona 0.23 (largest, theater), planner 0.12 (second), goal register -0.11 (now modest, realizing more than it shows once every blocked instance truly blocks); harness -0.02 evidence / +0.01 capacity (executor sign flip). Maps are marginal contributions: phi_E sums to 1.48, empty coalition -0.48.
- Interaction (capacity game): planner-map +0.20 (synergy), map-memory -0.18 (redundancy), goal-register-planner 0 (additive). The evidence-game interactions (+0.30 / -0.25) ship alongside under `interaction_index_evidence_game`; the v5.3-and-earlier paper reported the evidence values against a capacity claim, corrected in the v6 audit pass. Persona swap 1.0 (identity by design), model swap 0.61. Boundary sweep (capacity, enclosure form) -0.20 to 1.0. Richness-agency dissociation over four systems (complexity carries no signal for agency; the earlier -0.68 correlation was an artifact of a chaotic walker that walked through walls, fixed in the v6 pass, and the 4-point coefficient was dropped from the paper in v7). Seed sweep block range 0.73-0.88. Map robustness (v8, 2026-07-21): across 20 redrawn instance sets the persona's L is constant at 0.230769 (pure-channel theorem) and the largest in the assemblage in 19 of 20 draws; the exception is seed 12, where the planner's earned L (0.2686 max) passes it; goal-register L ranges -0.541 to -0.056 (always the modest pole). Invariants strengthened: persona dummy now checked per-coalition over all 2^6 (max error 0.0), rest-zero renamed to its tested scope. Persona-swap key renamed capacity_persona_identity (no substitution executed; identity by construction). All keys in `simulation/output/results.json`.
