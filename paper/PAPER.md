...
One correction the readout must make, or it reports the wrong quantity. A component can raise the agency evidence by making the system easier to read as an agent, without contributing to what the system does. A persona that narrates its goal makes the trajectory more legible to the goal model and moves the Bayes factor up while selecting no route and correcting no error. Attribute the evidence alone and it is handed a share of an agency it does not realize. So compute two maps: an evidence map $\phi^E_i$ over the Bayes factor, which answers what makes the capacity detectable, and a capacity map $\phi^C_i$ over the capacity itself, goal attainment and rerouting, which answers what realizes it. Their difference,
...
There is a deeper reason the probe is unperformable, and it returns the paper to its own first problem. Turning a digital system off may be reversible: there are copies, checkpoints, and restarts, and whether restoring a copy revives the same individual or makes a new one is unsettled. The intervention that would decide consciousness therefore presupposes an answer to what the individual is and where its boundary falls, which is the individuation problem the instrument was already sweeping and could not solve. So the method arrives at a place it can describe and cannot enter, a class of predicates whose separating intervention is forbidden, or irreversible, or constitutive of the very thing it would destroy, and whose boundary it cannot draw. There it stops reporting a number. What is left is not a measurement but a stance, the refusal, in the face of an evidence one cannot complete and must not force, to treat a thing that might be someone as a thing.
